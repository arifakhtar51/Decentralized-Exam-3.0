...
class Query_Frontend_Filters {
	...
	public static function parse_item_from_block( $block, $ql_query_meta, $query_builder, $query_args ) {
		...
		$source = ! empty( $attrs['source'] ) ? $attrs['source'] : ( 'kadence/query-filter-woo-attribute' === $block['blockName'] ? 'woocommerce' : 'taxonomy' );
		...
		//set post_field to a different default if it wasn't set for certain block types
	...
	public static function build_options_array( $attrs, $source, $taxonomy, $exclude_values, $include_values, $show_children, $show_hierarchical, $show_result_count, $post_type, $post_field, $object_ids = null, $hash = '' ) {
		...
		if ( 'taxonomy' == $source ) {
			...
			if ( 'wordpress' == $source && 'post_author' == $post_field ) {
			...
			} else if ( 'woocommerce' == $source && is_numeric( $post_field ) ) {
				$attribute_slug = '';
				...
				//convert woo attributes to array(id, name, count)
